...
def create_serverlog():
    ...
    with open("server.log", "w", encoding="utf-8") as log:
        for i in range(30):
            messages = [
            ...
            log.write(
                f"{datetime.now().isoformat(timespec='seconds')} | {random.choice(levels)} | "
...
def register_operation(operation_name: str):
    def decorator_register_operation(original_function):
        ...
        def wrapper_register_operation(*args, **kwargs):
            ...
            print(message, end="")
            with open("auditory.log", "a", encoding="utf-8") as log:
                log.write(message)
            return result
...
def analyze_logs(logs: dict) -> dict:
    ...
    most_active_hour = max(registers_by_hour, key=registers_by_hour.get)
    ratio_error = round(len(all_critic_services) / total_registers, 4)
    return {
